...
- `likesCount` and `commentsCount` come from Prisma `_count`; do not maintain counters manually. `commentsCount` is a filtered relation count (`comments: { where: { deletedAt: null } }`), so soft-deleted comments are not counted.
- **Soft delete.** `Post` and `Comment` carry `deletedAt DateTime?` (migration `soft_delete`). Both repositories define `NOT_DELETED = { deletedAt: null }` and apply it to every read, including `findById` (now `findFirst`), search, the repost-once check and comment lists; deletes go through `softDeleteById`. Comments of a deleted post are not marked separately: they become unreachable together with the post (§2.3) because every comment operation first checks the post through `PostService.findPublishedPost`, and they would come back if the post were restored. Likes and subscriptions remain hard-deleted by design — they are toggles with nothing referencing them. Reposts of a deleted original stay visible and keep `originalPostId` (previously the FK `SetNull` wiped it).
